...
def add_products(data):
    requests.post(f'{os.environ.get("API_URL")}add-products/', json=data)
...
def update_product(product: dict):
    ...
    :return:
    '''
    response = requests.post(f'{os.environ.get("API_URL")}update/', json={"product_to_update": product})
    return response
...
def add_product_to_promo(product_url: str, store: str, category: str, certain: bool) -> bool:
    ...
    response = requests.post(f'{os.environ.get("API_URL")}add-product-to-promo/', json=data)
...
def check_product(name, url, price, availability, products_in_db: dict, store: str, category: str):
    if url not in products_in_db.keys():
        return {
            "new": {
                "url": url,
                "name": name,
    ...
    elif url in products_in_db.keys():
        price_in_db = products_in_db[url]['price']
        if price_in_db:
        ...
        if price != price_in_db:
            ...
            if availability and "last_best_price" in r.json():
                ...
                if price < price_in_db and (1 - price/price_in_db) * 100 > 16.0:
                    ...
                    response = requests.get(f'{os.environ.get("API_URL")}search-best-price', json={
                    ...
                    if price <= lowest_price_all_stores:
                        ...
                        if lowest_from_ceneo and price < lowest_from_ceneo and (1 - price/lowest_from_ceneo) * 100 > 15.0:
